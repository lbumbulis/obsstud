...
for (iter in start.iter:stop.iter) {
  ...
  if (is.correct) {
    m.cox <- coxph(Surv(u.prev, u, Z1) ~ x2 + x3:B + x4 + v, data=dat.cox, method="breslow")
    coef.order <- c(1, 4:6, 2:3)
  } else {
    m.cox <- coxph(Surv(u.prev, u, Z1) ~ x2 + x3:B + v, data=dat.cox, method="breslow")
    m.cox.robust <- coxph(
      Surv(u.prev, u, Z1) ~ x2 + x3:B + v + cluster(i),
      data = dat.cox, method = "breslow", robust=T
    )
    coef.order <- c(1, 3:5, 2)
  }
