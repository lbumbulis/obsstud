...
for (iter in start.iter:stop.iter) {
  ...
  if (model.type == "cox") {
    ...
    coef.est.filename <- paste0("mcox_cause", cause, "_est.csv")
    coef.var.filename <- paste0("mcox_cause", cause, "_var.csv")
    
    H0.hat <- basehaz(m.cox, centered=F)
    saveRDS(H0.hat, file=paste0("./basehaz/basehaz_cause", cause, "_iter", iter, ".rds"), compress="bzip2")
  } else {
    if (cause == 2) {
